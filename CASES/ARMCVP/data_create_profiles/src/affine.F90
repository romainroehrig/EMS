subroutine affine(klev1,pcoo1,pval1,klev2,pcoo2,pval2,pts,pps)
! --------------------------------------------------------------
...
! Mais si pcoo1 est croissant (resp. décr.) pcoo2 doit être également décroissant (resp. décr.).
! --------------------------------------------------------------
...
do jlev=1,klev2
	...
	elseif(zcoo >= zcoo1(klev1)) then
		...
		enddo
		if(.not.llok) then
			write(*,fmt=*) 'affine/ERREUR: lors de l''interpolation d''un tableau de taille ',klev1,' sur un tableau de taille ',klev2
