...
f = cdms2.open('/Users/romainroehrig/Desktop/TMP/KUANG_SST28_DEPHY/Out_klevel.nc')
...
var3Dforc = ['pressure_forc','temp_rad','temp_nudging','qv_nudging','qv_adv','u_nudging','v_nudging']
var2D = ['ts','ps']
...
titles['qv_adv'] = 'Specific humidity advection'
...
for lev0 in range(1,91):
    ...
    g = cdms2.open('KUANG_SST{0}_QP{2}_l{1:0>2}_DEPHY.nc'.format(int(SST),int(lev0),ampl_name),'w')

    for var in var3Dini + var3Dforc + var2D:
        g.write(data[var])

    g.comment = 'Initial conditions for RCE simulation SST = {0}C with positive qv perturbation at level {2} ({3:8.2f} hPa); Initial state from the last 20 days of RCEMIP/SST={1}K'.format(SST,SST+273.15,lev0,datain['pf'][lev0]/100.)
    g.reference = 'Herman and Kuang (2013, JAMES)'
    g.author = 'R. Roehrig'


    g.case = 'RCE/KUANG_SST{0}_QP{2}_l{1:0>2}'.format(SST,lev0,ampl_name)
...
for lev0 in range(1,91):
    ...
    g = cdms2.open('KUANG_SST{0}_QM{2}_l{1:0>2}_DEPHY.nc'.format(int(SST),int(lev0),ampl_name),'w')

    for var in var3Dini + var3Dforc + var2D:
        g.write(data[var])

    g.comment = 'Initial conditions for RCE simulation SST = {0}C with negative qv perturbation at level {2} ({3:8.2f} hPa); Initial state from the last 20 days of RCEMIP/SST={1}K'.format(SST,SST+273.15,lev0,datain['pf'][lev0]/100.)
    g.reference = 'Herman and Kuang (2013, JAMES)'
    g.author = 'R. Roehrig'


    g.case = 'RCE/KUANG_SST{0}_QM{2}_l{1:0>2}'.format(SST,lev0,ampl_name)
    g.startDate = '{year:4d}{month:02d}{day:02d}000000'.format(year=tmin.year,month=tmin.month,day=tmin.day)
    g.endDate = '{year:4d}{month:02d}{day:02d}000000'.format(year=tmax.year,month=tmax.month,day=tmax.day)
