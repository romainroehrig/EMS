...
var3Dforc = ['pressure_forc','trad','temp_nudg','qv_nudg','tadvh']
var2D = ['ts','ps']
...
titles['tadvh'] = 'Temperature horizontal advection'
...
for lev0 in range(1,91):
    ...
    g = cdms2.open('KUANG_SST{0}_perturb{1:0>2}_DEPHY.nc'.format(int(SST),int(lev0)),'w')

    for var in var3Dini + var3Dforc + var2D:
        g.write(data[var])

    g.comment = 'Initial conditions for RCE simulation SST = {0}C with perturbation at level {2} ({3:8.2f} hPa); Initial state from the last 20 days of RCEMIP/SST={1}K'.format(SST,SST+273.15,lev0,datain['pf'][lev0]/100.)
    g.reference = 'Herman and Kuang (2013, JAMES)'
    g.author = 'R. Roehrig'


    g.case = 'RCE/KUANG_SST{0}_perturb{1}'.format(SST,lev0)
