...
class Case:
    ...
    def convert2SCM(self,time=None,lev=None,levtype=None):
        ...
        for var in self.varlist:
            if not(var in ['ps',]):
                if lvert and ltime:
                    ...
                    dataout[var] = interpol(self.variables[var],levout=levout)
                elif ltime:
                    print 'ERROR: case unexpected for interpolation'
                    print 'ERROR: lvert:', lvert
                    print 'ERROR: ltime:', ltime
                    sys.exit()
                    dataout[var] = interpol(self.variables[var],timeout=timeout)
                else:
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('theta_adv' in caseSCM.varlist):
            ...
            if not('thetal_adv' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('temp_adv' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('theta_rad' in caseSCM.varlist):
            ...
            if not('thetal_rad' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('temp_rad' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('qt_adv' in caseSCM.varlist):
            ...
            if not('rv_adv' in caseSCM.varlist):
            ...
            if not('rt_adv' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] == 1:
            ...
            if not('qv_adv' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] > 0:
            ...
            if not('theta_nudging' in caseSCM.varlist):
            ...
            if not('thetal_nudging' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] > 0:
            ...
            print 'assume thetal_nudging=theta_nudging'
            if not('thetal_nudging' in caseSCM.varlist):
                caseSCM.add_variable('thetal_nudging',dataout[var].data,lev=lev,levtype=levtype,levid='lev',time=time,timeid='time')
            ...
            if not('temp_nudging' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] > 0:
            ...
            if not('qt_nudging' in caseSCM.varlist):
            ...
            if not('rv_nudging' in caseSCM.varlist):
            ...
            if not('rt_nudging' in caseSCM.varlist):
        ...
        if att in self.attlist and self.attributes[att] > 0:
            ...
            if not('qv_nudging' in caseSCM.varlist):
