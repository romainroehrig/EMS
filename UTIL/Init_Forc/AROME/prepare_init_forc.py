...
#        datain[var], dataout_forc[var] = prep_forcing(var)        
    ...
    for var in config.variablesAux.keys():
        print >>g, var
        if var == 'SURFZ0.FOIS.G' and case.attributes['z0']>0. :
            print >>g, str(9.80665*float(case.attributes['z0']))
        elif var == 'SURFGZ0.THERM'  and case.attributes['z0']>0. :
            print >>g, str(9.80665*float(case.attributes['z0'])/10.)
        else:
