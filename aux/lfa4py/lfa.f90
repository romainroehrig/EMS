...
!
!-------------------------------------------------
! Nombre maximal d'unit�s logiques possibles.
! Ne sert qu'� dimensionner le tableau recevant
! les noms en clair des fichiers des unit�s logiques ouvertes par LFA,
...
!	endif
	...
	call assign(classign)
#endif
	open(kul,file=cdnomf,form='unformatted',status='old',convert='BIG_ENDIAN')
	read(kul) iversionesb
	iversion=iversionesb
	!
...
elseif(cdtypo(1:iltypo) == 'W') then
	...
	call assign(classign)
#endif
	open(kul,file=cdnomf,form='unformatted',status='replace',convert='BIG_ENDIAN')
	!
...
elseif(cdtypo(1:iltypo) == 'S') then
	...
	call assign(classign)
#endif
	open(kul,form='unformatted',status='scratch',convert='BIG_ENDIAN')
	!
	! Ecriture de l'en-tete du fichier LFA.
	!
	iversionesb=iversion
...
elseif(cdtypo(1:iltypo) == 'A') then
	...
	call assign(classign)
#endif
	open(kul,file=cdnomf,form='unformatted',status='old',position='append',convert='BIG_ENDIAN')
...
do jarg=1,jparg
	...
	if(clnoma == ' ') then
		...
		!
		! -------------------------------------------------
		! On force la pr�cision d'�criture des entiers et r�els.
		! -------------------------------------------------
		!
		if(cltype(1:1) == 'R') then
			!
		...
		!
		! -------------------------------------------------
		! L'utilisateur fournit-il un fichier d'entr�e?
		! -------------------------------------------------
		!
...
if(.not.llex) then
	...
	call assign(classign)
#endif
	open(kul,file=cdnomf,form='unformatted',status='old',err=100,convert='BIG_ENDIAN')
...
if(iarg == 0) then
	...
	if(cllang() == 'FRA') then
		write(*,'(a)') ' '
		write(*,'(a)') 'Courbes de fr�quence et de r�partition d''un champ.'
		write(*,'(a)') ' '
		...
		write(*,'(a)') 'de la valeur VALn � VALx'
		write(*,'(a)') ' '
		write(*,'(a,i3)') 'D�faut de NILES: ',inile
		write(*,'(a,i3)') 'D�faut de VALn: le min du champ, d�termin� � partir des donn�es lues sur le fichier.'
		write(*,'(a,i3)') 'D�faut de VALx: le max du champ, d�termin� � partir des donn�es lues sur le fichier.'
		write(*,'(a)') ' '
		write(*,'(a)') 'Il en effectue la courbe de fr�quence, et la courbe de r�partition, donn�e par ses n-iles.'
		write(*,'(a)') 'Il sort 3 fichiers:'
		write(*,'(a)') ' '
		write(*,'(a)') '	- lfafreq.tmp.freq: courbe de fr�quence.'
		write(*,'(a)') '	  Deux colonnes dans ce fichier: la 1�re est le centre de la classe,'
		write(*,'(a)') '	  la 2�me la fr�quence correspondante.'
		write(*,'(a)') '	  On entend ici par fr�quence la densit� de probabilit� de cette classe;'
		write(*,'(a)') '	  son unit� est l''inverse de celle des donn�es d''entr�e.'
		write(*,'(a)') '	  La somme des �l�ments de la deuxi�me colonne, pond�r�e par la largeur'
		write(*,'(a)') '	  moyenne de chaque bande, donne 1..'
		write(*,'(a)') '	  UNITE DE LA COLONNE 1: celle des donn�es d''entr�e.'
		write(*,'(a)') '	  UNITE DE LA COLONNE 2: 1/(celle des donn�es d''entr�e).'
		write(*,'(a)') ' '
		write(*,'(a)') '	- lfafreq.tmp.rep: courbe de r�partition.'
		write(*,'(a)') '	  Deux colonnes dans ce fichier: soit x la 1�re, y la 2�me; '
		write(*,'(a)') '	  alors p(X<x)=y.'
		write(*,'(a)') '	  UNITE DE LA COLONNE 1: celle des donn�es d''entr�e.'
		write(*,'(a)') '	  UNITE DE LA COLONNE 2: sans unit� (et entre 0 et 1).'
		write(*,'(a)') ' '
		...
		write(*,'(a)') '	  UNITE DE L''UNIQUE COLONNE: celle des donn�es d''entr�e.'
		write(*,'(a)') ' '
	else
		write(*,'(a)') ' '
...
!
!-------------------------------------------------
! Lecture de cet article de donn�es.
!-------------------------------------------------
!
allocate(zdat(ilong))
...
inblig=ilong
print*,'lfafreq:'
print*,'	',inblig,' r�els lus.'
print*,'	',knile,' niles d�sir�s.'
zmin=minval(zdat)
zmax=maxval(zdat)
if(pmin /= pindef) then
	!
...
do while(.not.llok)
	...
	!
	!-------------------------------------------------
	! On r�partit le champ dans les classes d�finies par le tableau zbord.
	!-------------------------------------------------
	!
	...
	do jval=1,inblig
		...
		do jbord=1,ibord
			if(zdat(jval) <= zbord(jbord)) then
				!
				!-------------------------------------------------
				! La valeur r�elle zdat(jval) est entre
				! zbord(jbord) et zbord(jbord-1).
				! On augmente donc de 1 le nombre d'�l�ments au sein de la classe
				! jbord.
				!-------------------------------------------------
				!
		...
		if(.not.lltro) then
			!
			!-------------------------------------------------
			! La valeur courante n'a �t� affect�e � aucune classe.
			! C'est qu'elle est �gale au max.
			!-------------------------------------------------
			!
	...
	!
	!-------------------------------------------------
	! Si une classe donn�e a une probabilit� sup�rieure
	! au seuil zpros, on la casse en sous classes, de fa�on
	! it�rative jusqu'� ce que la probabilit� devienne inf�rieure
	! au seuil ou qu'il y ait trop de classes.
	!-------------------------------------------------
	!
	zpros=0.2 ! probabilit� seuil.
	zpros=2. !!!!!! gol  probabilit� seuil.
	ibordtmp=0
	zbordtmp(0)=zbord(0)
	llok=.true.
	...
	do jbord=1,ibord
		...
		if(zproba > zpros) then
			!
			!-------------------------------------------------
			! La classe a une probabilit� sup�rieure
			...
			if(ibord+isous < jpbord.and.ibouc < 6) then
				!
				!-------------------------------------------------
				! Eu �gard � la taille statique du tableau
			...
			!
			!-------------------------------------------------
			! La classe a une probabilit� inf�rieure
...
do jbord=1,ibord
	...
	zdens=zfreq(jbord)/real(inblig)/(zbord(jbord)-zbord(jbord-1)) ! densit� de probabilit�.
...
enddo
close(21)
print*,'	Fichier g�n�r�: lfafreq.tmp.freq'
close(22)
print*,'	Fichier g�n�r�: lfafreq.tmp.rep'
!
!-------------------------------------------------
...
enddo
close(24)
print*,'	Fichier g�n�r�: lfafreq.tmp.cl'
end
subroutine nile(pbord,prepar,krepar,knile,pnile)
! --------------------------------------------------------------
! **** *NILE* Fourniture des n-iles d'un champ, connaissant la fct de r�partition.
! --------------------------------------------------------------
! Sujet:
! Arguments explicites:
! Arguments implicites:
! Methode:
! Il s'agit simplement d'inverser la fonction de r�partition.
! Externes:
! Auteur:   1999-01, J.M. Piriou.
! Modifications:
! --------------------------------------------------------------
! En entree:
! pbord: valeur du champ au bord des classes de r�partition.
! prepar: fct de r�partition.
! krepar: taille du tableau prepar.
! knile: nombre de n-iles d�sir�s.
! En sortie:
! pile: tableau des n-iles calcul�s.
! --------------------------------------------------------------
