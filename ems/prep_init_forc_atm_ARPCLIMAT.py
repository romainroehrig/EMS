...
def prep_init_forc_atm(
        ...
        save_init=True, save_forc=True, **kwargs):
    ...
    vert_grid_name = vertical_grid.split('.')[0]

    with open('nam1D_{0}'.format(vert_grid_name), 'w') as g:
        g.write('&NAM1D')
        g.write('  LMAP    = .FALSE.,\n')
        g.write('  IFLEV   = {0},\n'.format(int(nlev_out)))
    ...
    if save_init:
        fileout = 'init_{0}.nc'.format(vert_grid_name)
        with nc.Dataset(fileout, 'w', format='NETCDF3_CLASSIC') as g:
    ...
    if lforc:
        ...
        if save_forc:
            fileout = 'forc_{0}.nc'.format(vert_grid_name)
            with nc.Dataset(fileout, 'w', format='NETCDF3_CLASSIC') as g:
